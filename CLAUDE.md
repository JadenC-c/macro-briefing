...
10. **各板块固定格式**：
   ...
   - 重大事件跟踪 → 卡片列表，【升级/持续/缓和】标签，事件（背景）/ 最新（日期+进展）/ 现状 / 影响品种 各一行
   - 品种分析 → 编号列表，宏观面判断统一格式 `短期【偏X】· 中期【偏X】`
   - 每个 h2 标题下方放 `[【返回目录】](#目录)` 链接
11. **颜色规则**（HTML/PDF）：利空 = 绿色，利多 = 红色，中性 = 橙色。符合中国交易习惯（红涨绿跌）。品种宏观面判断 badge：偏空/中性偏空 = 绿底绿字（`.bearish`），偏多/中性偏多 = 红底红字（`.bullish`），中性 = 黄底棕字（`.neutral`）
...
| 品种分析 | verdict badge：偏空/中性偏空=绿底绿字，偏多/中性偏多=红底红字，中性=黄底棕字 |
| 页脚 | 只放页码（X/Y），无免责声明 |
